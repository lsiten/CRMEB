...
## 22. 营销列表与会员券（LSIT-14 下一批）

秒杀、拼团、砍价列表与会员券沿用第21节的 order-management 容器、卡片、字阶、品牌色和44PX按钮。秒杀场次使用可横向滚动的 order-tabs，选中态同时有底色和文本；切换场次清空旧列表，失败可以重试，分页失败保留前页。会员券仅展示服务端金额、适用范围、日期和状态，游客先登录，不虚构领取或使用成功。

活动详情沿用同组卡片与 CommerceImage，规格通过原生按钮选择，缺货规格仍可查看；加载、请求失败、未开始、售罄、提交中与账号变化均有明确反馈。秒杀购买与新开拼团先创建服务端活动购物车，再进入结算；当前数量固定1件。砍价助力、参团和预售购买流程尚未补齐，详情不再用无购物车ID的跳转假装进入结算。没有新增动画或视觉令牌。主要验收者为手机单手购物者与弱网用户；H5浏览器验证和小程序构建分别记录，真实微信交易仍需环境联调。

## 22. 商品发现与排序

分类页复用既有750PX容器、品牌色、圆角、14/12PX文字与44PX触控尺寸。分类下方增加可换行的综合、销量、价格排序按钮；选中同时用品牌底色和明确文字标记，价格明确从低到高/从高到低。排序由服务端执行，不对已下载页做局部排序。关键词展示独立结果说明与清除入口，来自独立搜索页的关键词面向全部分类。

首次加载/失败与追加加载/失败分开：首次失败不显示旧筛选商品；追加失败保留已加载商品，列表底部提供原页重试；末页明确“已显示全部商品”。切换分类、排序或关键词重置到第一页。复用现有Empty、CommerceImage，不新增动效或依赖。
