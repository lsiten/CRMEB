...
收藏和足迹复用 CommerceImage、Modal 与 order-management 卡片布局（750PX 容器、14PX 正文、12PX 辅助、22PX 标题和至少44PX按钮）。收藏提供加载、分页、错误重试、登录入口、失效商品、确认取消和提交中状态；取消成功后刷新服务端列表，失败保留记录。详情收藏使用服务端状态，提交期间禁用重复操作。浏览记录提供加载、空态、失败重试和分页；删除能力等待服务端限定日志类型后再启用，避免误删其他业务日志。

本批足迹清空由默认关闭的构建开关控制；后端安全补丁集成并联调后启用。复用 Modal 确认、order-alert 错误和44PX按钮；提交期间锁定关闭与重复提交，失败保留列表并支持取消核对、重试。优惠券复用 Loading、白卡内错误与重试，只有成功加载才展示筛选空态；重试保留当前标签。
