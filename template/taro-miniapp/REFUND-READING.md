...
- 金额取 refund_price（兼容 pay_price，服务端将其赋为 refund_price），单位元，不做价格重算。只有type=6且未撤销标“已退金额”，其他标“申请退款金额”。商品展示服务端返回的 cart_num 数量（详情split分支会将其改为surplus_num，完整拆单售后仍未验收），不使用 surplus_num/refund_num 差值；申请页仍使用剩余可退数量。
