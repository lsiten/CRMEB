...
import { addToCart, canAddToCart } from '../../services/cart';
import './index.scss';
...
const GoodsPage = () => {
  ...
  return <View className='page'><Text className='title'>商品分类</Text><Input className='search' value={keyword} onInput={(event) => setKeyword(event.detail.value)} placeholder='搜索商品' /><View className='categories'>{['全部', '新品', '热卖', '家居'].map((item) => <Text className={category === item ? 'active' : ''} key={item} onClick={() => setCategory(item)}>{item}</Text>)}</View><View className='grid'>
    {visible.map((product) => <View className='product card' key={product.id}><Image mode='aspectFill' src={product.image} /><Text>{product.name}</Text><Text className='primary'>¥{product.price.toFixed(2)}</Text><Button disabled={!canAddToCart(product)} onClick={() => { if (!canAddToCart(product)) { void Taro.showToast({ title: '已达到购买上限', icon: 'none' }); return; } addToCart(product); void Taro.showToast({ title: '已加入购物车', icon: 'success' }); }}>加入购物车</Button></View>)}
  </View></View>;
};
