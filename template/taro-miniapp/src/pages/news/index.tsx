...
const NewsPage = () => {
  ...
  return <View className='page newsPage'>{banners.length > 0 && <Swiper className='banner' autoplay circular indicatorDots>{banners.map((item) => <SwiperItem key={item.id} onClick={() => void Taro.navigateTo({ url: `/pages-extra/news-detail/index?id=${item.id}` })}><OptimizedImage src={item.images[0] ?? ''} mode='aspectFill' /></SwiperItem>)}</Swiper>}<View className='tabs'><Text className={active === 0 ? 'active' : ''} onClick={() => select(0)}>热门</Text>{categories.map((category) => <Text key={category.id} className={active === category.id ? 'active' : ''} onClick={() => select(category.id)}>{category.title}</Text>)}</View>{error && <View className='empty'>资讯暂时无法加载</View>}{!error && articles.length === 0 && <View className='empty'>暂无资讯</View>}{articles.map((item) => <View className='article card' key={item.id} onClick={() => void Taro.navigateTo({ url: `/pages-extra/news-detail/index?id=${item.id}` })}><View className='articleText'><Text className='title'>{item.title}</Text><Text className='meta'>{item.createdAt ?? ''}</Text></View>{item.images[0] && <OptimizedImage src={item.images[0]} mode='aspectFill' />}</View>)}</View>;
};
