...
export default function RefundDetailPage() {
  ...
  return <View className='order-management'><Text className='order-heading'>售后详情</Text>
    ...
    {resource.loading ? <View className='order-panel'>正在加载售后详情…</View> : refund && <>
      <View className='order-panel'><Text className='order-section-title'>{refund.title}</Text>{refund.message && <Text className='order-muted'>{refund.message}</Text>}<Text className='order-muted'>{refund.type === 6 && !refund.cancelled ? '已退金额' : '申请退款金额'}</Text><Text className='order-amount'>¥{refund.amount.toFixed(2)}</Text><Text className='order-muted'>售后单号：{refund.id}</Text><Text className='order-muted'>订单号：{refund.orderId || '暂未提供'}</Text></View>
