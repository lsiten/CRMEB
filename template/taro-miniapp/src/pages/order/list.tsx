...
export default function OrderListPage() {
  ...
  const resource = usePagedResource(tab, (page) => getOrders(tab, page));
  return <View className='order-management'><View className='order-between'><Text className='order-heading'>我的订单</Text><Button onClick={() => Taro.navigateTo({ url: '/pages/order/refunds' })}>退款 / 售后</Button></View>
    ...
    {resource.items.map((order) => <View className='order-panel' key={order.id}>
      <View className='order-between'><Text>{order.statusText ?? order.status}</Text><Text className='order-amount'>¥{order.total.toFixed(2)}</Text></View><Text className='order-muted'>订单号：{order.id}</Text>
    ...
    {!resource.loading && !resource.error && !resource.items.length && <View className='order-panel order-empty'><Text>暂无订单</Text><Button onClick={() => Taro.switchTab({ url: '/pages/goods/index' })}>去购物</Button></View>}
