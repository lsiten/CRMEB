...
import { getClipboardFallback, resolvePaymentStatus } from '../../services/platform';
import './index.scss';

export default function PayPage() { const { params } = useRouter(); const orderId = params.orderId ?? ''; const [status, setStatus] = useState('pending'); const [loading, setLoading] = useState(false); const [clipboard, setClipboard] = useState(getClipboardFallback(orderId, false));
  const refreshStatus = async (): Promise<void> => { const server = await queryPayment(orderId); setStatus(resolvePaymentStatus('ok', server.status)); };
  const pay = async () => { setLoading(true); try { const result = await requestPayment({ orderId, method: 'wechat' }); if (process.env['TARO_ENV'] === 'h5') { setStatus('failed'); await Taro.showToast({ title: result.payParams ? '请在服务端配置 H5 支付跳转' : 'H5 暂不支持微信支付', icon: 'none' }); return; } if (result.payParams) await Taro.requestPayment(result.payParams as Taro.requestPayment.Option); await refreshStatus(); } catch { setStatus('failed'); } finally { setLoading(false); } };
  const copyOrderId = async (): Promise<void> => { try { await Taro.setClipboardData({ data: orderId }); setClipboard(getClipboardFallback(orderId, true)); } catch { setClipboard(getClipboardFallback(orderId, false)); } };
  useEffect(() => { let active = true; let timer: ReturnType<typeof setTimeout> | undefined; const poll = async () => { try { const result = await queryPayment(orderId); if (!active) return; setStatus(resolvePaymentStatus('ok', result.status)); if (result.status === 'pending') timer = setTimeout(() => void poll(), 2000); } catch { if (active) timer = setTimeout(() => void poll(), 3000); } }; if (orderId) void poll(); return () => { active = false; if (timer) clearTimeout(timer); }; }, [orderId]);
