...
export default function PayPage() { const { params } = useRouter(); const orderId = params.orderId ?? ''; const [status, setStatus] = useState('pending'); const [loading, setLoading] = useState(false);
  const pay = async () => { setLoading(true); try { const result = await requestPayment({ orderId, method: 'wechat' }); if (result.payParams) await Taro.requestPayment(result.payParams as Taro.requestPayment.Option); } catch { setStatus('failed'); } finally { setLoading(false); } };
