...
function parseRefund(value: unknown): Refund {
  ...
  const refundedPrice = row['refunded_price'];
  const refundedAmount = refundedPrice == null || typeof refundedPrice === 'string' && !refundedPrice.trim() ? null : apiAmount(refundedPrice);
