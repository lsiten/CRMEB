...
export async function uploadImage(filePath: string): Promise<string> {
  ...
  try {
    response = await Taro.uploadFile({
      ...
      header: { 'Authori-zation': `Bearer ${token}`, ...(tenant.token ? { 'X-Tenant-Token': tenant.token } : {}) }, timeout: 30000,
    });
  ...
  if (isTenantInvalid(body)) { await tenantSession.renew(tenant); throw new TenantError('TENANT_UNAVAILABLE'); }
